	...
	class pVerify_Main
	{
		...
		public function fn_pVerify_submit_verifyKeysform() {
			...
			if(!empty($record_id) && $record_id > 0 ){
				...
				$updated = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}$tblname SET client_api_id='$client_api_id', client_secret='$client_secret' WHERE id = $record_id"));
				
				if( $updated !== false ){

					wp_send_json_success( array( 'message' => __( 'Details updated successfully.', 'pVerify' ) ) );
				}else if( $updated == 0 ){
					wp_send_json_error( array( 'message' => __( 'Please enter different values to update the record.', 'pVerify' ) ) );
				}
				
			}else{

				$inserted = $wpdb->query("INSERT INTO {$wpdb->prefix}$tblname (client_api_id, client_secret) VALUES ('$client_api_id', '$client_secret')"  );
				if( $inserted !== false ){
					wp_send_json_success( array( 'message' => __( 'Details saved successfully.', 'pVerify' ) ) );
				}
			}
